...
const inviteAdmin = async (req, res) => {
  try {
    ...
    const newAdmin = await admin.createAccount(req.body.email, req.id);
    if (newAdmin[0]) {
      const msg = {
...
const signupAdmin = async (req, res) => {
  try {
    ...
    const check = await admin.setupAccount(req.body);
    if (check[0]) {
...
const login = async (req, res) => {
  try {
    ...
    const admin = new Admin(req.body.email);
    const check = await admin.authenticateAdmin(req.body);
    if (check[0]) {
      res.cookie('token', check[1], { expiresIn: '1d', httpOnly: true });
    ...
    return responseHandler(
      res,
      check[1] || 'Incorrect username or passwsord',
      400,
      true,
      ''
    );
...
const getPendingVendor = async (req, res) => {
  try {
    const admin = new Admin(req.email);
